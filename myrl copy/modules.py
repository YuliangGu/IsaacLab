...
class ActorCriticAug(ActorCritic):
    ...
    def __init__(self, *base_args,
                 ...
                 ctx_dim: int = 0,
                 feat_dim: Optional[int] = 64,  # ObsEncoder output dim; defaults to obs dim
                 **kwargs):
        """Augmented ActorCritic compatible with rsl-rl.

        - Shares a learnable encoder between actor and critic.
        - Optionally injects previous action (additive) and context (FiLM/concat).
        - Preserves rsl-rl constructor API by forwarding all base args.
        """
        ...
        try:
            print(f"[ActorCriticAug] encoder_out={self.encoder.out_dim} head_in={head_in} ctx_dim={self.ctx_dim} prev_act={int(self.use_prev_action)}")
        except Exception:
            pass
