...
from myrl.utils_core import BYOLSeq, sample_byol_windows, ObsEncoder, ActionEncoder
...
class PPOWithBYOL(PPO):
    ...
    def __init__(self,
            ...
            byol_window: int = 32,
            byol_batch: int = -1,                 # -1 -> auto: 2*minibatch_size capped at 512
            byol_tau_start: float = 0.99,
            byol_tau_end: float = 0.999,
            byol_z_dim: int = 64,
            byol_proj_dim: int = 64,
        ...
        #overr policy
        if not isinstance(self.policy, ActorCriticAug):
            raise ValueError("PPOWithBYOL requires the policy to be an instance of ActorCriticAug")
        ...
        # IMPORTANT: freeze target networks
    ...
    def _zero_ctx(self):
        ...
        self.policy.set_belief(self.ctx_h)
    ...
    def act(self, obs):
        """ Overriden to infer context first. """
        ct = self._infer(obs)
    ...
    def update(self):
        ...
        # set BYOL to training mode
        self.byol.train() # NOTE: this will set batchnorm to train mode

        # TODO: anneal tau
        if self.byol_tau_start != self.byol_tau_end:
            pass

        # Get raw data from storage for BYOL sampling
        obs_raw = self.storage.observations.get("policy", None) # for byol 
        dones_raw = self.storage.dones.squeeze()
        ...
        b_ctx = self.ctx_buf.reshape((-1, self.byol_z_dim))
        ...
        for epoch in range(self.num_learning_epochs):
            ...
            for start in range(0, batch_size, mini_batch_size):
                ...
                # Reset context at episode starts to avoid cross-episode leakage.
                ...
                entropy = self.policy.entropy[:mini_batch_size]  # assume all entropies are the same within minibatch
                ...
                if self.byol_lambda > 0:
                    v1, v2, _, _= sample_byol_windows(
                        obs_raw,
                        dones_raw,
                        self.byol_window,
                        self.byol_batch if self.byol_batch > 0 else min(2*mini_batch_size, 512),
